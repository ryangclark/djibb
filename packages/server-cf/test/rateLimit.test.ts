 ...
 *
 *   1. Anonymous entity writes hit `RL_ANON_WRITE` (keyed by IP): the
 *      (limit+1)th write is a 429 with `Retry-After` + the shared body shape.
 *   2. Authenticated writes use the *looser* `RL_ACCT_WRITE` (keyed by
 *      account): well past the anon cap, still no 429 — proving the branch,
 *      not the shared window.
 *   3. The OAuth callback hits `RL_AUTH_IP` (keyed by IP): the (limit+1)th
