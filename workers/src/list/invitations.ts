...
/**
 * Read one pending invite by (identity_kind, identity_value).
...
export function getPendingInvite(
    ...
    const rows = sql
        .exec(
            `SELECT identity_kind, identity_value, role, inviter_account_id,
                    time_created, time_expires, version
...
/** List all pending invites in this DO. */
...
/**
 * Insert a pending invite. Caller is responsible for normalizing
 * `identity_value` (see `normalizeIdentityValue`) and computing
 * `time_expires`. Throws on PK conflict — the mutator should check
 * `getPendingInvite` first if it wants idempotent semantics.
 */
...
/**
 * Hard-delete a pending invite. Returns true if a row was deleted.
 * Idempotent for missing rows (returns false).
 */
