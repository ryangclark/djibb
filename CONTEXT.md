...
**Quantity covers both checkboxes and counts.** `value` and `target_value` are `number | boolean`; the discriminator is `unit`. When `unit === 'boolean'`, the Item is a checkbox: `value: false → true` flips it. When `unit` is anything else (`'tbsp'`, `'gallon'`, `'count'`, etc.), the Item carries an actual count and `min_value`/`max_value` may apply. Completion is universal: an Item is done when `value === target_value` (numeric or boolean). This keeps a single Item shape across recipes, packing lists, instructions, and wishlists. `superRefine` short-circuits the numeric invariants when `unit === 'boolean'`.
