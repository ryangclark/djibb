...
**A Workspace is itself a djibb entity.** Same DO-backed substrate as List and Template — one `DjibbList` DO class serves all entity kinds (one DO namespace, addressed by entity id), with workspace-specific behavior (`is_personal` flag, Island hex coords, the cascade dispatcher from ADR 0008) living in an internal workspace module the DO delegates to, gated on the top-level `type` discriminator (`list | template | workspace`). There is deliberately no `DjibbWorkspace` subclass — ADR 0011 rejected subclass-per-type, and ADR 0026 records why single-namespace addressing makes a separate DO class a data migration, not a refactor. Workspace membership is, in the long run, an entry in the workspace's own `authorization_rules.authorized_accounts` — the same shape Lists already use — resolving the `@UPGRADE` note in `auth/rules.ts`. `AccountWorkspace` membership rows are a transitional artifact of the prior workspaces-as-D1-rows model. (See "djibb uses itself" under Design principles.)
